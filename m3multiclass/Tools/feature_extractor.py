...
def extractFeatures(img, imagepath):
    ...
    if 'CANNY' in cfg.featuresToExtract:
        cannyFeats = extractCannyfeatures(img)
        feats.extend(cannyFeats)
    if 'HOGRGB' in cfg.featuresToExtract:
        img = io.imread(imagepath, as_gray = False)
        for i in [0,1,2]:
            hogFeats = extractHOGfeatures(img[:,:,i])
            feats.extend(hogFeats)
...
def extractCannyfeatures(img):

    img = ndi.gaussian_filter(img, 4)
    edges = canny(img)
    edges = np.asarray(edges).flatten() * 1
