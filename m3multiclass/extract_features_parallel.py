...
def run():
    ...
    for i in range(cfg.num_classes):
        outputFolder = 'Features/'+featuresFolder+'/'+cfg.labels[i]
        inputFolder = cfg.datasetRoot+'/'+cfg.labels[i]
        if not os.path.exists(outputFolder):
            os.makedirs(outputFolder)
        print 'Extracting features from images in: '+cfg.labels[i]
        extractAndStoreFeatures(inputFolder, outputFolder, '.jpg', cfg.labels[i])



def extractAndStoreFeatures(inputFolder, outputFolder, extension, currentClass):
    ...
    Parallel(n_jobs=2)(delayed(extractFeaturesList)(filename,currentClass,inputFolder,outputFolder) for filename in imagesList)



def extractFeaturesList(filename,currentClass,inputFolder,outputFolder):
    ...
    outputpath = outputFolder+'/'+filename+'_'+currentClass+'.feat'
